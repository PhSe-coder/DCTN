...
def register(name: str,
             ...
             exist_ok: bool = False) -> Callable[[Type[_T]], Type[_T]]:
    """
    ...
    def add_subclass_to_registry(subclass: Type[_T]) -> Type[_T]:
        ...
        if name in registry:
            if exist_ok:
                ...
                message = (f"Cannot register {name} as {name}; "
...
class Embedding(TokenEmbedder):
    ...
    def __init__(
        ...
        if weight is not None:
            raise Exception("Embedding was constructed with both a weight and a pretrained file.")
...
class SpanExtractorWithSpanWidthEmbedding(SpanExtractor):
    ...
    def __init__(
        ...
        if num_width_embeddings is not None and span_width_embedding_dim is not None:
            self._span_width_embedding = Embedding(num_embeddings=num_width_embeddings,
                                                   embedding_dim=span_width_embedding_dim)
        elif num_width_embeddings is not None or span_width_embedding_dim is not None:
            raise Exception("To use a span width embedding representation, you must"
    ...
    def forward(
        ...
        span_embeddings = self._embed_spans(sequence_tensor, span_indices, sequence_mask,
                                            span_indices_mask)
    ...
    def _embed_spans(
        ...
        if not self._use_exclusive_start_indices:
            if sequence_tensor.size(-1) != self._input_dim:
                raise ValueError(f"Dimension mismatch expected ({sequence_tensor.size(-1)}) "
...
def bucket_values(distances: torch.Tensor,
    ...
    logspace_index = (distances.float().log() /
                      math.log(2)).floor().long() + (num_identity_buckets - 1)
...
def flatten_and_batch_shift_indices(indices: torch.Tensor, sequence_length: int) -> torch.Tensor:
    ...
    if torch.max(indices) >= sequence_length or torch.min(indices) < 0:
        raise Exception(f"All elements in indices should be in range (0, {sequence_length - 1})")
